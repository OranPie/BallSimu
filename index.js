...
const TEAM_DEF = [
    ["RED", [0.08, 0.08], RED],
    ["BLUE", [0.92, 0.08], BLUE],
    ["GREEN", [0.08, 0.92], GREEN],
    ["YELLOW", [0.92, 0.92], YELLOW],
];
...
const EVENT_REWARDS = [15000, 150000, 300000, 600000, 1200000, 2400000, 5000000];
const REWARD_WEIGHTS = [0.5, 0.2, 0.2, 0.05, 0.03, 0.01, 0.01];
...
class Particle {
    constructor(pos, color, velocity, lifetime = 30) {
        this.pos = [...pos];
        this.color = color;
...
class TeamBase extends Entity {
    ...
    draw(ctx) {
        ...
        const terr_width = ctx.measureText(territory_text).width;
        ctx.fillStyle = 'rgb(200,200,200)';
        ctx.fillText(territory_text, this.pos[0] - terr_width / 2, this.pos[1] + 80);
        if (this.target_lock) {
            ctx.beginPath();
...
class Game {
    constructor() {
        this.teams = TEAM_DEF.map(def => new TeamBase(...def));
        this.bullets = [];
        this.balls = [];
        this.running = true;
        this.frame_count = 0;
    ...
    trigger_random_event() {
        ...
        const reward_level = weighted_random(EVENT_REWARDS.length, REWARD_WEIGHTS);
        const reward = EVENT_REWARDS[reward_level];
    ...
    update() {
        ...
        if (this.frame_count - this.last_event_frame > 100) {
            if (Math.random() < 0.1) {
